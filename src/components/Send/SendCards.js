...
const SendCards = (props) => {
  ...
  const handleEdit = () => {
    if (user.edit1) {
      ...
      const associateprint = associate.associateData.find(data => data.associateName == edit.editData.receiverName);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const trimmedBase64 = screenShot.substring(0 + "data:image/jpeg;base64,".length);
  ...
  return (
    ...
    >
        ...
        <Dialog open={showPreview}>
          ...
          <Tooltip title="Close">
            <IconButton
              ...
              aria-label="close"
              sx={{ position: "absolute", top: "8px", right: "30px" }}
            > <CloseIcon />
            </IconButton>
          </Tooltip>
          <DialogContent
          ...
          <Button style={{ background: "#0071e3", color: "#FCFCFC", borderRadius: "5px", textTransform: "none", marginBottom: "2px", marginLeft: "465px", width: "70px" }} onClick={handleSubmit}>
            Submit
          </Button>
      ...
      <Box
        id="previewScreenShot" className="data-container-hidden" ref={hiddenDataContentRef}>
        <div
          style={{
            width: "480px",
            height: "390px",
